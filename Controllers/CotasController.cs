...
public static class CotasController
{
    ...
    public static async Task<List<Cota>> GetCotasHandler(this RouteGroupBuilder _, AppDbContext db) =>
        await db.Cotas
            .Include(c => c.Grupo)
            .Include(c => c.Cliente)
            .ToListAsync();

    public static async Task<IResult> GetCotaByIdHandler(this RouteGroupBuilder _, int id, AppDbContext db)
    {
        var cota = await db.Cotas
            .Include(c => c.Grupo)
            .Include(c => c.Cliente)
    ...
    public static async Task<IResult> CreateCotaHandler(this RouteGroupBuilder _, Cota cota, AppDbContext db)
    {
        var grupo = await db.Grupos.FindAsync(cota.GrupoId);
        if (grupo == null)
            return Results.NotFound("Grupo não encontrado");
    ...
    public static async Task<IResult> UpdateCotaHandler(this RouteGroupBuilder _, int id, Cota cotaAtualizada, AppDbContext db)
    {
        var cota = await db.Cotas.FindAsync(id);
        if (cota is null)
            return Results.NotFound("Cota não encontrada");
    ...
    public static async Task<IResult> DeleteCotaHandler(this RouteGroupBuilder _, int id, AppDbContext db)
    {
        var cota = await db.Cotas.FindAsync(id);
        if (cota is null)
            return Results.NotFound("Cota não encontrada");
